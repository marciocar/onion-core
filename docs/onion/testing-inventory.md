...
|----------|---:|----------|
| Famílias na bancada | **192** | `grep -cE '^_family ' .claude/validation/lint-selftest.sh` |
| Sítios de asserção (**não** asserções executadas) | **1243** | `grep -cE '^\s*(record_pass\|record_fail\|record_skip) ' .claude/validation/lint-selftest.sh` |
...
A distinção não é formalismo. Há **1243** sítios estáticos de asserção e a última
