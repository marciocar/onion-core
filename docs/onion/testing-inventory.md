...
|----------|---:|----------|
| Famílias na bancada | **193** | `grep -cE '^_family ' .claude/validation/lint-selftest.sh` |
| Sítios de asserção (**não** asserções executadas) | **1252** | `grep -cE '^\s*(record_pass\|record_fail\|record_skip) ' .claude/validation/lint-selftest.sh` |
...
| — sem teste | **0** | idem |
| Scripts de validação | **80** | `git ls-files '.claude/validation/*.sh'` |
...
A distinção não é formalismo. Há **1252** sítios estáticos de asserção e a última
