...
| Sítios de asserção (**não** asserções executadas) | **1145** | `grep -cE '^\s*(record_pass\|record_fail\|record_skip) ' .claude/validation/lint-selftest.sh` |
...
A distinção não é formalismo. Há **1145** sítios estáticos de asserção e a última
